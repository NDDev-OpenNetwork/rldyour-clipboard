...
/// KDE's hint (and the GTK-safe `text/` variant KeePassXC is adopting) is
/// what KeePassXC, KWallet and Klipper agree on; the NSPasteboard types are
/// the macOS equivalents; 1Password refuses the standard marks and uses its
/// own.
const SENSITIVE: &[&str] = &[
...
mod tests {
    ...
    fn recognises_the_hints_password_managers_actually_set() {
        ...
        assert!(is_sensitive("X-KDE-PasswordManagerHint"));
        assert!(is_sensitive("text/x-kde-passwordManagerHint"));
        assert!(is_sensitive("application/x-nspasteboard-concealed-type"));
        assert!(is_sensitive("org.nspasteboard.TransientType"));
        assert!(is_sensitive("org.nspasteboard.AutoGeneratedType"));
        assert!(is_sensitive("com.agilebits.onepassword"));
